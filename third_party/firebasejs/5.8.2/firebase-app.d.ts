...
declare namespace firebase {
  ...
  interface User extends firebase.UserInfo {
    ...
    isAnonymous: boolean;
    linkAndRetrieveDataWithCredential(credential: firebase.auth.AuthCredential):
    ...
    linkWithPopup(provider: firebase.auth.AuthProvider):
        Promise<firebase.auth.UserCredential>;
    ...
    reauthenticateAndRetrieveDataWithCredential(
        ...
        Promise<firebase.auth.UserCredential>;
    reauthenticateWithCredential(credential: firebase.auth.AuthCredential):
        Promise<void>;
    reauthenticateWithPhoneNumber(
        phoneNumber: string,
        applicationVerifier: firebase.auth.ApplicationVerifier):
        Promise<firebase.auth.ConfirmationResult>;
    reauthenticateWithPopup(provider: firebase.auth.AuthProvider):
        Promise<firebase.auth.UserCredential>;
    reauthenticateWithRedirect(provider: firebase.auth.AuthProvider):
        Promise<void>;
    refreshToken: string;
    reload(): Promise<void>;
    sendEmailVerification(actionCodeSettings?: firebase.auth.ActionCodeSettings|
                          null): Promise<void>;
    toJSON(): Object;
    unlink(providerId: string): Promise<firebase.User>;
    updateEmail(newEmail: string): Promise<void>;
    updatePassword(newPassword: string): Promise<void>;
    updatePhoneNumber(phoneCredential: firebase.auth.AuthCredential):
        Promise<void>;
...
declare namespace firebase.functions {
  ...
  export type ErrorStatus =|'ok'|'cancelled'|'unknown'|'invalid-argument'|
...
declare namespace firebase.auth {
  interface ActionCodeInfo {
    data: {email?: string|null; fromEmail?: string | null;};
    operation: string;
  }

  type ActionCodeSettings = {
    android?:
        {installApp?: boolean; minimumVersion?: string; packageName: string;};
    handleCodeInApp?: boolean;
    iOS?: {bundleId: string}; url: string;
    dynamicLinkDomain?: string;
  };

  type AdditionalUserInfo = {
    isNewUser: boolean; profile: Object | null; providerId: string;
  ...
  interface Auth {
    ...
    sendSignInLinkToEmail(
        email: string,
        actionCodeSettings: firebase.auth.ActionCodeSettings): Promise<void>;
    ...
    signInAndRetrieveDataWithCustomToken(token: string):
        Promise<firebase.auth.UserCredential>;
    signInWithEmailAndPassword(email: string, password: string):
        Promise<firebase.auth.UserCredential>;
    signInAndRetrieveDataWithEmailAndPassword(email: string, password: string):
        Promise<firebase.auth.UserCredential>;
  ...
  class EmailAuthProvider extends EmailAuthProvider_Instance {
    ...
    static credential(email: string, password: string):
        firebase.auth.AuthCredential;
    static credentialWithLink(email: string, emailLink: string):
        firebase.auth.AuthCredential;
  }
  ...
  class RecaptchaVerifier_Instance implements
      firebase.auth.ApplicationVerifier {
    constructor(
        container: any|string, parameters?: Object|null,
  ...
  type UserCredential = {
    ...
    operationType?: string | null; user: firebase.User | null;
  };
...
declare namespace firebase.auth.Auth {
  ...
  var Persistence:
      {LOCAL: Persistence; NONE: Persistence; SESSION: Persistence;};
}
...
declare namespace firebase.database {
  interface DataSnapshot {
    ...
    exists(): boolean;
    exportVal(): any;
    forEach(action: (a: firebase.database.DataSnapshot) => boolean | void):
        boolean;
...
declare namespace firebase.messaging {
  interface Messaging {
    ...
    onMessage(
        ...
        completed?: firebase.CompleteFn): firebase.Unsubscribe;
    onTokenRefresh(
        nextOrObserver: firebase.NextFn<any>|firebase.Observer<any>,
        error?: firebase.ErrorFn,
        completed?: firebase.CompleteFn): firebase.Unsubscribe;
    requestPermission(): Promise<void>;
    setBackgroundMessageHandler(callback: (payload: any) => Promise<any>| void):
        void;
...
declare namespace firebase.storage {
  ...
  interface Reference {
    ...
    put(data: any|any|any, metadata?: firebase.storage.UploadMetadata):
  ...
  interface SettableMetadata {
    ...
    customMetadata?: {[/* warning: coerced from ? */ key: string]: string;}|
        null;
  }
  ...
  var StringFormat: {
    BASE64: StringFormat; BASE64URL: StringFormat; DATA_URL: StringFormat;
    RAW: StringFormat;
  };

  type TaskEvent = string;
  var TaskEvent: {STATE_CHANGED: TaskEvent;};

  type TaskState = string;
  var TaskState: {
    CANCELED: TaskState; ERROR: TaskState; PAUSED: TaskState;
    RUNNING: TaskState;
    SUCCESS: TaskState;
  };
  ...
  interface UploadTask {
    ...
    on(event: firebase.storage.TaskEvent,
       nextOrObserver?: firebase.Observer<any>|null|((a: Object) => any),
       error?: ((a: Error) => any)|null,
       complete?: (firebase.Unsubscribe)|null): Function;
    pause(): boolean;
    resume(): boolean;
...
declare namespace firebase.firestore {
  ...
  export interface PersistenceSettings {
     ...
     * shared execution of queries and latency-compensated local document
     * updates across all connected instances.
     *
     * To enable this mode, `experimentalTabSynchronization:true` needs to be
     * set globally in all active tabs. If omitted or set to 'false',
   ...
   * the local cache or attempt to fetch results from the server and fall back
   * to the cache (which is the default).
   */
  export interface GetOptions {
     ...
     * updated if the server request succeeds. Also note that
     * latency-compensation still takes effect, so any pending write operations
     * will be visible in the returned data (merged into the server-provided
     * data).
     *
     * Setting to 'cache' causes Firestore to immediately return a value from
     * the cache, ignoring the server completely (implying that the returned
     * value may be stale with respect to the value on the server.) If there is
     * no data in the cache to satisfy the `get()` call,
     * `DocumentReference.get()` will return an error and `QuerySnapshot.get()`
  ...
  export class DocumentReference {
     ...
     * fail if you are offline and the server cannot be reached. This behavior
     * can be altered via the `GetOptions` parameter.
     *
  ...
  export interface SnapshotOptions {
     ...
     * and cause these values to change once the server result becomes
     * available.
     *
  ...
  export class FieldValue {
     ...
     * already exists on the server. All instances of each element specified
     * will be removed from the array. If the field being modified is not
