...
export class TriageModel extends ThreadListModel {
  ...
  constructor(updateTitle: any, private vacation_: string, labels: Labels, settings_: Settings,
    ...
    this.mailProcessor_ = new MailProcessor(settings_, this, queueSettings_, labels, updateTitle);
  }
  ...
  async addThread(thread: Thread) {
    ...
    this.threadsToSerialize_.push(new PlainThreadData(thread.id, thread.historyId));
  ...
  hasBestEffortThreads() {
    return Boolean(this.bestEffortThreads_ && this.bestEffortThreads_.length !== 0);
  }
  ...
  async fetch() {
    ...
    let makeTimeLabels = this.labels.getMakeTimeLabelNames().filter((item) => item != Labels.PROCESSED_LABEL);
