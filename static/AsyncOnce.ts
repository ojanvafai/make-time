...
export class AsyncOnce<T> {
  ...
  constructor(
    asyncAction: (value?: {} | PromiseLike<NonNullable<T>>) => PromiseLike<NonNullable<T>>,
  ) {
    this.queued_ = [];
  ...
  async do(): Promise<T> {
    ...
    if (this.isDoing_) return new Promise((resolve) => this.queued_.push(resolve));
    ...
    for (let resolve of this.queued_) resolve(this.value_);
    delete this.queued_;
