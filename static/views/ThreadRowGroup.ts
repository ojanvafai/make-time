...
export class ThreadRowGroup extends ThreadRowGroupBase {
  ...
  private setupRangeSlider_(slider: HTMLInputElement, tickmarks: HTMLDataListElement) {
    ...
    this.addEventListener(AfterFocusRowEvent.NAME, () => this.updateSliderPositionFromFocus_());
  }
  ...
  private appendHeader_() {
    ...
    this.groupNameContainer_.addEventListener('click', () =>
      this.setManuallyCollapsed_(!this.collapsed_),
    );
    ...
    this.groupNameContainer_.append(this.expander_, this.name, this.rowCountDisplay_);
  ...
  private updateRowCount_() {
    if (!this.groupNameContainer_ || !this.rowCountDisplay_) return;

    // -1 is a magic value for allowed count to hide the count of threads
    // entirely.
    if (this.allowedCount_ === -1) return;
    ...
    let text;
    if (overLimit) text = ` (${count}/${this.allowedCount_})`;
    else text = ` (${count})`;
  ...
  setCollapsed(collapsed: boolean, force?: boolean) {
    if (!this.allowCollapsing_ || (!force && this.manuallyCollapsed_)) return;
  ...
  private rowsChanged_(rows: ThreadRow[]) {
    if (rows.length !== this.rowContainer_.childElementCount) return true;
    for (var i = 0; i < rows.length; i++) {
      if (rows[i] !== this.rowContainer_.children[i]) return true;
    }
    return false;
  }
  ...
  private updateSliderPositionFromFocus_() {
    ...
    if (focusedIndex !== -1) assert(this.slider_).value = String(focusedIndex);
  }
  ...
  render() {
    ...
    if (!rowListChanged && !collapseChanged) return [];
    ...
    if (this.collapsed_) {
      ...
      if (this.slider_) {
        ...
        if (effectiveMode === ThreadRowGroupRenderMode.ShowOnlyHighlightedRows) {
    ...
    for (let row of this.rows_) {
      if (
        previousRow ? row.previousSibling !== previousRow : row !== this.rowContainer_.firstChild
      ) {
        if (previousRow) previousRow.after(row);
        else this.rowContainer_.prepend(row);
      }
  ...
  selectRows(select: boolean) {
    ...
    for (let child of rows) {
      if (child.checked !== select) child.setChecked(select);
      if (!select) child.clearFocusImpliesSelected();
    }
