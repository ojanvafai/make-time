...
export class SettingsView extends View {
  ...
  constructor(private settings_: Settings) {
    ...
    this.basicSettings_.style.cssText = `
      ...
      border-radius: 5px;
    `;
    SettingsView.appendSettings(this.basicSettings_, this.settings_, Settings.fields);
    ...
    this.calendarSortView_.addEventListener('change', () => this.handleChange_());
    ...
    buttonContainer.style.cssText = `
      ...
      flex-wrap: wrap;
    `;
    buttonContainer.append(this.saveButton_, mailFilters, calendarFilters, helpButton);
  ...
  static createInput(settings: Settings, field: Setting) {
    ...
    if (field.values) {
      ...
      if (settings.has(field.key)) currentValue = settings.getNonDefault(field.key);
      ...
      else if (settings.has(field.key)) input.value = settings.getNonDefault(field.key);
  ...
  static appendSettings(container: HTMLElement, settings: Settings, fields: Setting[]) {
  ...
  async save_() {
    ...
    let [themeSelect, priorityInboxSelect] = this.querySelectorAll(
      'select',
    ) as NodeListOf<HTMLSelectElement>;
    updates[ServerStorage.KEYS.THEME] = themeSelect.selectedOptions[0].value;
    updates[ServerStorage.KEYS.PRIORITY_INBOX] = priorityInboxSelect.selectedOptions[0].value;

    updates[ServerStorage.KEYS.QUEUES] = this.queues_.getAllQueueDatas();
    updates[ServerStorage.KEYS.CALENDAR_SORT] = this.calendarSortView_.getAllCalendarSortDatas();
