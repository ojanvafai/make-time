...
const serverStorageDocumentReference = db
  .collection('server-storage-collection')
  .doc('server-storage-doc');
...
const mktimePriority1 = new MockGmailLabel('mktime-priority-id-1', `${PRIORITY_LABEL_NAME}/1`);
const mktimeRootLabel = new MockGmailLabel('mktime-root-id', MAKE_TIME_LABEL_NAME);
const mktimeSoftMuteLabel = new MockGmailLabel('mktime-soft-mute-id', SOFT_MUTE_LABEL_NAME);
...
test('populateGmailLabelsToPush_ soft muted', async () => {
  ...
  const mailProcessor = new MailProcessor(mockSettings);
  jest
    .spyOn(mailProcessor as any, 'ensureLabelsExist_')
  ...
  expect(gmailLabelUpdate.removeLabelIds).toEqual(['INBOX', mktimeRootLabel.id]);
});
...
test('populateGmailLabelsToPush_ not in inbox hasLabel and hasPriority', async () => {
  const mockSettings = new Settings(new MockServerStorage());
  const mailProcessor = new MailProcessor(mockSettings);
  const allMessages: gapi.client.gmail.Message[] | undefined = [new MockGmailMessage('1', [])];
  const gmailLabelUpdate = new GmailLabelUpdate(['1']);
  await mailProcessor['populateGmailLabelsToPush_'](
    allMktimeGmailLabels,
    mktimeRootLabel.id,
  ...
  expect(gmailLabelUpdate.addLabelIds).toEqual(['INBOX', mktimeRootLabel.id]);
  expect(gmailLabelUpdate.removeLabelIds).toEqual([]);
  ...
  await mailProcessor['populateGmailLabelsToPush_'](
    ...
    allMessages,
    { hasPriority: true },
    secondGmailLabelUpdate,
  );
  expect(secondGmailLabelUpdate.addLabelIds).toEqual(['INBOX', mktimeRootLabel.id]);
  expect(secondGmailLabelUpdate.removeLabelIds).toEqual([]);
});

test('populateGmailLabelsToPush_ in inbox, !hasLabel, !muted, and !hasPriority', async () => {
  const mockSettings = new Settings(new MockServerStorage());
  const mailProcessor = new MailProcessor(mockSettings);
  const allMessages: gapi.client.gmail.Message[] | undefined = [
  ...
  expect(gmailLabelUpdate.addLabelIds).toEqual([]);
  expect(gmailLabelUpdate.removeLabelIds).toEqual([mktimeRootLabel.id, 'INBOX']);
});

test('populateGmailLabelsToPush_ hasLabel pushes label and not priority', async () => {
  const mockSettings = new Settings(new MockServerStorage());
  jest
    .spyOn(mockSettings, 'get')
    .mockImplementation((setting: string) => setting === ServerStorage.KEYS.PUSH_LABELS_TO_GMAIL);
  ...
  jest
    ...
    .mockImplementation(mockEnsureLabelsExist as any);
  jest
    .spyOn(mailProcessor as any, 'getAllMktimeGmailLabels_')
  ...
  jest.spyOn(mailProcessor as any, 'getLabelName_').mockImplementation(mockGetLabelName as any);
  ...
  expect(gmailLabelUpdate.addLabelIds).toEqual(['INBOX', mktimeRootLabel.id, mktimeLabel1.id]);
  expect(gmailLabelUpdate.removeLabelIds).toEqual([]);
});

test('populateGmailLabelsToPush_ hasPriority pushes label and priority', async () => {
  const mockSettings = new Settings(new MockServerStorage());
  jest
    .spyOn(mockSettings, 'get')
    .mockImplementation((setting: string) => setting === ServerStorage.KEYS.PUSH_LABELS_TO_GMAIL);
